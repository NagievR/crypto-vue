...
export default {
  ...
  methods: {
    ...
    addTicker() {
      ...
      const nameFormatted = this.tickerName.toUpperCase();

      const tickerExists = this.tickerList.find(obj => obj.name === nameFormatted);
      if (tickerExists) {
        this.errors.nameExists = true;
        return;
      }
      
      this.requestData(nameFormatted);
      ...
      this.tickerName = '';
      this.tickerList.push(newData);
      this.updateTickersOnStorage();
    },
    removeTicker(id) {
      this.tickerList = this.tickerList.filter(obj => obj.id !== id);
      this.updateTickersOnStorage();
    },
    updateTickersOnStorage() {
      const oldPriceRemoved = this.tickerList.map(obj => ({...obj, price: '...'}));
      localStorage.setItem('tickerList', JSON.stringify(oldPriceRemoved));
    },
  ...
  created() {
    ...
    if (storedTickers) {
      const parced = JSON.parse(storedTickers);
      this.tickerList = parced;
      this.tickerList.forEach(obj => this.requestData(obj.name));
    }
